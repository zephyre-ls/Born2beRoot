...
- Swap area = partition utilise comme memoire virtuelle, permet d etendre la RAM sb. Toujours en avoir une askip
...
- nosuid = empeche l exe de fichier avec le bit SUID(set User ID) ou SGID(set group id) = fichier ne peux pas s exe avec les privileges d un autre useur, renforce secu sur les systeme multi utilisateurs
